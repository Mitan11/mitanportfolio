...
export async function GET(req, { params }) {
  ...
  try {
    ...
    const data = await GeneralSettings.findById(id);
...
export async function PUT(req, { params }) {
  ...
  if (!adminUser || !adminUser.isAdmin) return NextResponse.json({ success: false, error: 'Not authorized as admin' }, { status: 403 });
    ...
    const data = await GeneralSettings.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
    });
    if (!data) return NextResponse.json({ success: false, error: 'Not found' }, { status: 404 });
...
export async function DELETE(req, { params }) {
  ...
  if (!adminUser || !adminUser.isAdmin) return NextResponse.json({ success: false, error: 'Not authorized as admin' }, { status: 403 });
    ...
    const { id } = await params;
    const deleted = await GeneralSettings.deleteOne({ _id: id });
